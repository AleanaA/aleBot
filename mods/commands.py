...
class Commands(Cog):
    ...
    async def profile(self, ctx, *user: discord.Member):
        ...
        embed.add_field(name="Joined Server", value=user.joined_at.strftime("%B %d %Y @ %I:%M:%S %p"), inline=True)
        embed.add_field(name="Account Created", value=user.created_at.strftime("%B %d %Y @ %I:%M:%S %p"), inline=True)
