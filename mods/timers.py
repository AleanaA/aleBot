...
class RemindMe:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.timers = fileIO("data/timers.json", "load")
        self.units = {"second" : 1,"minute": 60, "hour": 3600, "day": 86400, "week": 604800, "month": 2592000, "year": 31104000}

    @commands.command(pass_context=True)
    async def settimer(self, ctx, who : discord.User, quantity : int, time_unit : str, *, text : str):
        ...
        self.timers.append({"ID" : who.id, "AUTHOR" : author.id, "FUTURE" : future, "TEXT" : text})
        await ctx.send("{} set a timer for {} to say {}.".format(who.name, str(quantity), time_unit + s))
        fileIO("data/timers.json", "save", self.reminders)
